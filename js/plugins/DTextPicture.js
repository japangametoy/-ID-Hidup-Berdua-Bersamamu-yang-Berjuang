...
(() => {
    // 一時スキン名を保持
    Game_Screen.prototype.setTempDTextFrameSkin = function (name) {
        this._dTextTempFrameSkin = name || "";
    };

    // makeFrameWindow をフックして「次回だけ」上書きして消費
    const _makeFrameWindow = Sprite_Picture.prototype.makeFrameWindow;
    Sprite_Picture.prototype.makeFrameWindow = function (width, height) {
        _makeFrameWindow.call(this, width, height);

        // ベースのパラメータ参照（既定の param.frameWindowSkin は DTextPicture 本体が適用）
        const temp = $gameScreen._dTextTempFrameSkin;
        if (temp && this._frameWindow) {
            this._frameWindow.windowskin = ImageManager.loadSystem(temp);
            // 一回使ったら自動でクリア（＝「最初に出すときだけ」）
            $gameScreen._dTextTempFrameSkin = "";
        }
    };
})();
