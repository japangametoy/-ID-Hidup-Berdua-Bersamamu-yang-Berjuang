...
/* --- PictureCallCommon.js の末尾に追記 --- */
(() => {
    'use strict';
    // クリック連打を何フレーム抑止するか（60fps基準）。必要に応じて 3〜10 に調整。
    const SUPPRESS_FRAMES = 5;

    // 既存の requestSuppress を拡張し、複数フレーム抑止する
    const _requestSuppress = TouchInput.requestSuppress;
    TouchInput.requestSuppress = function () {
        this._suppressFrames = Math.max(this._suppressFrames || 0, SUPPRESS_FRAMES);
        _requestSuppress.call(this);
    };

    // 既存の suppressIfNeed を拡張し、_suppressFrames が残っている間は毎フレーム入力をクリア
    const _suppressIfNeed = TouchInput.suppressIfNeed;
    TouchInput.suppressIfNeed = function () {
        if (this._suppressFrames > 0) {
            this._currentState = this._createNewState();
            this._suppressFrames--;
            this._requestSuppress = false; // フラグは都度消す
            return;
        }
        _suppressIfNeed.call(this);
    };
})();
