...
(function() {
    ...
    // "first" を指定した場合、最上部に追加できるように拡張
    Window_Options.prototype.shiftCustomOptions = function (addPotion) {
        if (addPotion === 'first') {
            const newCommand = this._list.pop();
            this.addIndexForManoInputConfig(0);
            this._list.unshift(newCommand);
            return;
        }

        const targetCommand = this._list.find(command => command.symbol === addPotion);
        if (!targetCommand) return;

        const targetIndex = this._list.indexOf(targetCommand);
        const newCommand = this._list.pop();
        this.addIndexForManoInputConfig(targetIndex);
        this._list.splice(targetIndex, 0, newCommand);
    };
