 ...
 *
 * @param Map cursor speed
 * @default 3
 * @desc Specify cursor speed for map transfer mode.
 * 0 for standard function cursor.
 * @type number
 *
 ...
 *
 * @command watchSwitch
 * @text Add switch to the watch window
 *
 * @arg id
 * @text Switch ID
 * @type switch
 * @default 1
 *
 * @command watchVariable
 * @text Add variable to the watch window
 *
 * @arg id
 * @text Variable ID
 * @type variable
 ...
 * @text マップカーソル速度
 * @default 3
 * @desc マップ移動画面のカーソル速度を指定します。
 * 0 で標準のカーソル。
 * @type number
 *
 * @param Watch window font size
 * @text ウォッチウィンドウのフォントサイズ
 * @default 18
 * @desc ウォッチウィンドウのフォントサイズを指定します。
 * @type number
 * @min 1
 *
 * @param Watch highlight color
...
Params.mapCursorSpeed      = parseIntWithDefault(pluginParams['Map cursor speed'], 3)
...
const tilesetExName = PluginManager.parameters("TilesetEx").tilesetExName || "";
const regionCollisionExName = PluginManager.parameters("RegionCollisionEx").mapName || "";
...
PluginManager.registerCommand(PluginName, "watchSwitch", function (args) {
	const id = parseInt(args.id);
	if (!id) {
		console.error('[%s %s] Invalid ID: %s', Const.pluginCode, "watchSwitch", args.id);
		return;
	}
	const item = new DebugWatchItem();
	item.setupSwitch(id);
	$gameTemp.registerDebugWatchItem(item);
});

PluginManager.registerCommand(PluginName, "watchVariable", function (args) {
	const id = parseInt(args.id);
	if (!id) {
		console.error('[%s %s] Invalid ID: %s', Const.pluginCode, "watchVariable", args.id);
		return;
	}
	const item = new DebugWatchItem();
...
Object.defineProperties(TextManager.DebugUtil, {
    ...
    gold:            simpleGetter(isJp ? '所持金変更' : 'Edit gold'),
    goldCaption:            simpleGetter(isJp ? '所持金' : 'Gold'),
    ...
    forceSave:       simpleGetter(isJp ? '強制セーブ' : 'Force save'),
    forceLoad:       simpleGetter(isJp ? '強制ロード' : 'Force load'),
    removeSaveFile:  simpleGetter(isJp ? 'セーブファイル削除' : 'Remove save file'),
 ...
 * MZ用に追加
 */
Sprite_DebugMapDestination.prototype.setOffsets = function(x, y)
{
    this._offsetX = x;
    this._offsetY = y;
};
...
Sprite_Destination.prototype.updateAnimation = function() {
	_Sprite_Destination_updateAnimation.call(this);
	this.opacity *= 2; //不具合対策で薄くならないように調整
};
...
Sprite_DebugWatchUpdate.prototype.setup = function(x, y, width, height)
{
    ...
    this.blendMode = 1//Graphics.BLEND_ADD;
    this.move(x, y);
};
...
{
	const rect = new Rectangle(0, 0, width, this.fittingHeight(numLines));
...
Window_DebugCommand.prototype.initialize = function(x, y)
{
    ...
    const wh = this.fittingHeight(6);
	const rect = new Rectangle(x, y, ww, wh); 
...
const removeSaveFile = $plugins.some(plugin => Utils.extractFileName(plugin.name) === "RemoveSaveFile" && plugin.status);
Window_DebugCommand.prototype.makeCommandList = function()
{
    this.addCommand(TextManager.DebugUtil.itemNumber,   'itemNumber');
    this.addCommand(TextManager.DebugUtil.gold,         'gold');
    ...
    this.addCommand(TextManager.DebugUtil.forceSave,    'forceSave');
    this.addCommand(TextManager.DebugUtil.forceLoad,    'forceLoad');
	if (removeSaveFile) {
		this.addCommand(TextManager.DebugUtil.removeSaveFile,    'removeSaveFile');
	}
	...
	Window_DebugCommand.prototype.isCancelTriggered = function() {
		return (
			Window_Command.prototype.isCancelTriggered() ||
...
{
    var captionWidth = $gameSystem.windowPadding() * 2;
...
*/
Window_DebugNumberInput.prototype.windowHeight = function() {
	const rows = this._caption ? 2 : 1;
    if (ConfigManager.touchUI) {
        return this.fittingHeight(rows) + this.buttonSpacing() + 48;
    } else {
...
Window_NumberInput.prototype.buttonY = function() {
    return this.lineHeight() * (this._caption ? 2 : 1) + this.buttonSpacing();
};
...
const nameArray = [tilesetExName, regionCollisionExName].filter(mapName => !!mapName);
Window_DebugMapList.prototype.makeItemList = function() {
	this._data = $dataMapInfos.filter(info => {
		return !!info && !nameArray.some(mapName => info.name.includes(mapName))
	});
};
...
Window_DebugMapTilePosition.prototype.initialize = function(x, y, width)
{
	...
	const rect = new Rectangle(x, y, width, wh); 
...
{
	if (Params.mapCursorSpeed) {
		this._mapCursorPressedTime = 0;
		this._mapCursorTriggered = 0;
	}
	this._lastDir = [];
    this._data           = [];
...
Window_DebugMapTile.prototype.drawItemBackground = function() {};

Window_DebugMapTile.prototype.reselect = function() {
...
Window_DebugMapTile.prototype.maxVisibleItems = function() {
...
Window_DebugMapTile.prototype.contentsWidth = function() {
...
Window_DebugMapTile.prototype.updateArrows = function()
{
    ...
    // 左右アローの表示
	this._rightArrowSprite.visible = this._scrollX < this.maxScrollX();
    this._leftArrowSprite.visible  = this._scrollX > 0;
};

Window_DebugMapTile.prototype.smoothScrollRight = function(n) {
    this.smoothScrollBy(this.itemWidth() * n, 0);
};

Window_DebugMapTile.prototype.smoothScrollLeft = function(n) {
    this.smoothScrollBy(-this.itemWidth() * n, 0);
};
//スクロール範囲補正
Window_DebugMapTile.prototype.overallWidth = function() {
    return this.maxCols() * this.itemWidth();
};

const _Window_DebugMapTile_processWheelScroll =Window_DebugMapTile.prototype.processWheelScroll;
Window_DebugMapTile.prototype.processWheelScroll = function() {
	_Window_DebugMapTile_processWheelScroll.call(this);
...
Window_DebugMapTile.prototype.hitTest = function(x, y)
{
    ...
    var maxPageCols = Math.ceil(this.contentsWidth() / this.itemWidth());
...
Window_DebugMapTile.prototype.cursorDown = function(wrap) {
    ...
    } else if (wrap && maxRows >= 2) {
    	if (Params.mapCursorSpeed) this._mapCursorPressedTime = 0;
        this.smoothSelect((index + maxCols) % maxItems);
    }
};

Window_DebugMapTile.prototype.cursorUp = function(wrap) {
...
Window_DebugMapTile.prototype.cursorRight = function(wrap) {
    const index = this.index();
    const maxItems = this.maxItems();
    const maxCols = this.maxCols();
	if (maxCols >= 2) {
		if (index % maxCols + 1 < maxCols) {
			this.smoothSelect((index + 1) % maxItems);
		} else if (wrap) {
			if (Params.mapCursorSpeed) this._mapCursorPressedTime = 0;
			this.smoothSelect((index + 1 - maxCols) % maxItems);
		}
    }
};

Window_DebugMapTile.prototype.cursorLeft = function(wrap) {
    const index = Math.max(0, this.index());
    const maxItems = this.maxItems();
    const maxCols = this.maxCols();
	if (maxCols >= 2) {
		if (index % maxCols > 0) {
			...
			if (Params.mapCursorSpeed) this._mapCursorPressedTime = 0;
			this.smoothSelect((index - 1 + maxCols + maxItems) % maxItems);
		}
    }
};

Window_DebugMapTile.prototype.cursorPagedown = function() {
	const index = this.index();
	const maxItems = this.maxItems();
	if (this.topRow() + this.maxPageRows() < this.maxRows()) {
		this.smoothScrollDown(this.maxPageRows());
		const maxCols = this.maxCols();
		this.select(Math.min(index + maxCols * this.maxPageRows(), maxItems - maxCols + index % maxCols));
	}
};

Window_DebugMapTile.prototype.cursorPageup = function() {
	const index = this.index();
	if (this.topRow() > 0) {
		this.smoothScrollUp(this.maxPageRows());
		this.select(Math.max(index - this.maxCols() * this.maxPageRows(), 0));
	}
};
//斜め移動に対応（DS風）
if (Params.mapCursorSpeed) {
	const keyRepeatInterval = Params.mapCursorSpeed;
	Window_DebugMapTile.prototype.processCursorMove = function() {
		if (this.isCursorMovable()) {
			const lastIndex = this.index();
			if (!this._mapCursorTriggered) {
				this._mapCursorTriggered = Input.isTriggered("down") || Input.isTriggered("up") || Input.isTriggered("right") || Input.isTriggered("left");
			}
			if (Input.dir8 && this._mapCursorPressedTime === 0 || (this._mapCursorPressedTime >= Input.keyRepeatWait && this._mapCursorPressedTime % keyRepeatInterval === 0)) {
				if (Input.isPressed("down")) {
					this.cursorDown(this._mapCursorTriggered);
				}
				if (Input.isPressed("up")) {
					this.cursorUp(this._mapCursorTriggered);
				}
				if (Input.isPressed("right")) {
					this.cursorRight(this._mapCursorTriggered);
				}
				if (Input.isPressed("left")) {
					this.cursorLeft(this._mapCursorTriggered);
				}
				this._mapCursorTriggered = false;
			}
...
Window_DebugMapTile.prototype.ensureCursorVisible = function(smooth) {
	if (this._cursorAll) {
		this.scrollTo(0, 0);
	} else if ((this.innerHeight > 0 && this.row() >= 0) || (this.innerWidth > 0 && this.col() >= 0)) {
		...
		const scrollXMin = itemRight - this.innerWidth;
		const scrollYMin = itemBottom - this.innerHeight;
		let targetX = scrollX;
		let targetY = scrollY;

		if (scrollX > itemLeft) {
			targetX = itemLeft;
		} else if (scrollX < scrollXMin) {
			targetX = scrollXMin;
		}
		if (scrollY > itemTop) {
			targetY = itemTop;
		} else if (scrollY < scrollYMin) {
			targetY = scrollYMin;
		}
...
Window_DebugMapTile.prototype.getMapDisplaySize = function()
{
    return {
        width:
            (this._baseSprite.width) /
            this._baseSprite.scale.x,

        height:
            (this._baseSprite.height) /
            this._baseSprite.scale.y
    };
};
*/
...
{
    var sx = 0//this.padding;
    var sy = 0//this.padding;
    var sw = this.innerWidth//this.contentsWidth()//  - this.padding * 2;
    var sh = this.innerHeight//this.contentsHeight()// - this.padding * 2;
    ...
    this._clientArea.addChild(this._baseSprite);
};
...
Window_DebugMapTile.prototype.refreshTileset = function()
{
    ...
    //this._tilemap.refreshTileset()が存在しないので置き換え
    const bitmaps = [];

	const tilesetNames = this._map.tileset().tilesetNames;
...
const _Window_DebugMapTile_updateOrigin = Window_DebugMapTile.prototype.updateOrigin;
Window_DebugMapTile.prototype.updateOrigin = function() {
	_Window_DebugMapTile_updateOrigin.call(this);
	this.updateTilemap();
	this.updateDestinationPosition();
    this.origin.x = 0;
    this.origin.y = 0;
...
Window_DebugMapTile.prototype.updateTilemap = function()
{
	...
	const offsetX = this._tilemap.origin.x % this._tilemap.tileWidth;
	const offsetY = this._tilemap.origin.y % this._tilemap.tileHeight;
    this._gridSprite.x = -offsetX;
    this._gridSprite.y = -offsetY;
};
...
{
    if (!this._destinationSprite || !this._tilemap)
    {
        return;
    }

	const x = -this._tilemap.origin.x % this._tilemap.tileWidth;
	const y = -this._tilemap.origin.y % this._tilemap.tileHeight;
	this._destinationSprite.setOffsets(x, y);
...
{
    return 360 + $gameSystem.windowPadding() * 2;
};
...
Window_DebugWatch.prototype.createUpdateSprites = function()
{
    ...
    var count = Math.max(
        Math.floor((Graphics.boxHeight - this.y - $gameSystem.windowPadding() * 2) / lineHeight),
        1);
...
Window_DebugWatch.prototype.itemLineRect = function(index)
{
    // Window_Selectable の処理を流用
    return Window_Selectable.prototype.itemLineRect.call(this, index);
};

Window_DebugWatch.prototype.itemRectWithPadding = function(index) {
...
{
    // Window_Selectable の処理を流用
    return Window_Selectable.prototype.clearItem.call(this, index);
};
...
Window_DebugWatchItemEdit.prototype.drawItem = function(index)
{
    ...
    {
    case 'target':
        this.drawItemType(rect.x, rect.y - this.lineHeight() / 2, rect.width);
        this.drawItemId(rect.x, rect.y + this.lineHeight() / 2, rect.width);
        break;
...
Scene_Map.prototype.createDebugWatchWindow = function()
{
	...
	//チラつき防止
	if (this._debugWatchWindow.getWatchList().length > 0) {
...
Scene_Debug.prototype.createUpperWindowLayer = function()
{
	...
	this._upperWindowLayer.x = x;
	this._upperWindowLayer.y = y;
	this._upperWindowLayer.width = width;
	this._upperWindowLayer.height = height;
...
Scene_Debug.prototype.createCommandWindow = function()
{
    ...
    this._commandWindow.setHandler('gold',         this.onSelectGold.bind(this));
    ...
    this._commandWindow.setHandler("forceSave",    this.onForceSave.bind(this));
	this._commandWindow.setHandler('forceLoad',    this.onForceLoad.bind(this));
	this._commandWindow.setHandler('removeSaveFile', this.onRemoveSaveFile.bind(this));
	...
	const height = this._commandWindow.fittingHeight(Math.ceil(this._commandWindow.maxItems() / this._commandWindow.maxCols()));
	const maxHeight = Graphics.boxHeight - this._commandWindow.y;
	this._commandWindow.height = Math.min(height, maxHeight);
    this._commandWindow.forceSelect(0);
...
Scene_Debug.prototype.isReady = function()
{
    return _Scene_Debug_isReady.call(this) &&
		...
		(!tilesetExName || DataManager.isMapExLoaded()) &&
...
//所持金変更
Scene_Debug.prototype.onSelectGold = function() {
    this._titleWindow.setText(TextManager.DebugUtil.gold);
    this._numberWindow.setHandler('cancel', this.onGoldCancel.bind(this));

    const number = $gameParty.gold();
    const digits = $gameParty.maxGold().toString().length;
    this._numberWindow.setCaption(TextManager.DebugUtil.goldCaption);
    this._numberWindow.setAllowNegative(false);
    this._numberWindow.start(number, digits, this.onGoldOk.bind(this));
};

/**
 * 所持金入力の決定
 */
Scene_Debug.prototype.onGoldOk = function(number)
{
    // アイテム所持数の設定 (Game_Party に対して呼ぶ)
    const setGold = function(number)
    {
        const currNumber = this.gold();
        const amount = number - currNumber;
        this.gainGold(amount);
    };

    setGold.call($gameParty, number);
 ...
 * 所持金のキャンセル処理
 */
Scene_Debug.prototype.onGoldCancel = function() {
	this._numberWindow.close();
	this.activateCommandWindow();
};
...
Scene_Debug.prototype.onTransferMapListOk = function()
{
    ...
    this._postUpdateProc = function()
    {
    	...
    	if (tilesetExName) {
    		DataManager.loadMapExData(map.id);
    	}
    };
...
Scene_Debug.prototype.onForceSave = function() {
	SceneManager.push(Scene_Save);
};

Scene_Debug.prototype.onForceLoad = function() {
	SceneManager.push(Scene_Load);
};

Scene_Debug.prototype.onRemoveSaveFile = function() {
	SceneManager.push(Scene_Remove);
};

Scene_Debug.prototype.onGameEnd = function() {
