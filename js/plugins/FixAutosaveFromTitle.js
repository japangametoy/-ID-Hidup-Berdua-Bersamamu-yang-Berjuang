...
(() => {
    ...
    Scene_Map.prototype.shouldAutosave = function () {
        ...
        const fromTitle = SceneManager.isFromTitle();
        console.log(`[FixAutosaveFromTitle] shouldAutosave result: ${result}, isFromTitle: ${fromTitle}, previousClass: ${SceneManager._previousClass?.name}`);
        return result && !fromTitle;
    };

    SceneManager.isFromTitle = function () {
        return this._previousClass === Scene_Title
            || this._previousClass === Scene_Boot
            || this._previousClass === Scene_Load;  // ← 追記
    };
