...
/*:zh
 * @target MZ
 * @plugindesc 在地图上实现立体交叉（如桥梁）的插件
 * @author triacontane（原作者：Yoji Ojima）
 * @base PluginCommonBase
 * @orderAfter PluginCommonBase
 * @url
 *
 * @param overPathRegion
 * @text 立体交叉区域ID
 * @desc 设置为桥梁等立体交叉部分的区域ID。
 * @default 0
 * @type number
 *
 * @param overPathTerrainTag
 * @text 立体交叉地形标记
 * @desc 设置为桥梁等立体交叉部分的地形标记。
 * @default 0
 * @type number
 *
 * @param gatewayRegion
 * @text 立体交叉入口区域ID
 * @desc 设置为桥梁两端等立体交叉入口部分的区域ID。
 * @default 0
 * @type number
 *
 * @param gatewayTerrainTag
 * @text 立体交叉入口地形标记
 * @desc 设置为桥梁两端等立体交叉入口部分的地形标记。
 * @default 0
 * @type number
 *
 * @help OverpassTile.js
 *
 * 本插件可在地图上实现立体交叉（如桥梁）的表现效果。
 * 它是 RPG Maker MV 官方插件 “OverpassTile.js” 的 MZ 增强版。
 *
 * 支持不仅基于区域ID的判定，还可基于地形标记进行设置，
 * 并对事件触发与碰撞判定进行了扩展支持。
 * 注意：本插件不处理载具（船、飞艇）相关逻辑。
 *
 * 【通行判定规则】
 * ● 当角色位于桥的入口时  
 * 　→ 始终可以朝桥的立体交叉部分移动。  
 * ● 当角色位于桥的立体交叉部分时  
 * 　→ 若在桥上，可自由移动至桥的入口或其他立体交叉部分。  
 * 　→ 若在桥下，则无法朝桥的入口移动。  
 * ● 若不满足以上条件  
 * 　→ 按照地图原本的通行设定执行。
 *
 * 【事件触发与碰撞判定】
 * 插件支持高度差判定：  
 * - 位于不同高度的事件（如桥上与桥下）不会发生碰撞。  
 * - 同样，高度不同的事件也不会互相触发。  
 *
 * 使用此插件可轻松创建桥梁、立体道路、隧道等多层结构的地图表现效果。
 */
