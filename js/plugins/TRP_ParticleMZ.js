 ...
 * @help
 * 【更新履歴】
 * 1.34 2024/12/07 内部処理の変更
 * 1.33 2024/08/03 メンバー入替後の戦闘開始時に発生するエラー修正
 * 1.30 2023/01/22 他シーン用のパーティクルをクリア時の不具合修正
 * 1.29 2022/12/11 戦闘中に待機中アクターにステート付与時に発生する不具合の修正
 * 1.28 2022/12/05 戦闘不能などステート全解除時にパーティクルが残る不具合の修正
 * 1.27 2022/09/07 グループ機能追加時のセーブデータ互換性の対応
 ...
 * @param weatherZ
 * @text weather対象のZ軸値
 * @desc weather対象としたときのZ軸値のデフォ値。デフォルトはspriteset。ピクチャ下に表示したい場合はaboveなど
 * @default spriteset
 * @type string
 *
 * @param categoryClear
...
var weatherZ = parameters.weatherZ||'spriteset';
...
Game_Particle.prototype.defaultZIndexForType = function(targetType){
	...
	if(targetType < -1){
		...
		if(!isNaN(weatherZ)){
			z = Number(weatherZ)
		}else if(Z_INDEX[weatherZ]!==undefined){
			z = Z_INDEX[weatherZ];
		}else{
			z = Z_INDEX.spriteset;
		}
...
Game_Particle.prototype.particlePlay = function(eventId,id,target,name,z,x,y,image){
	var data = this.particleSet(eventId,id,target,name,z,x,y,image);
	if(data){
		data.clear = true;
	}
};
...
Game_Particle.prototype.particleUpdate = function(args){
    ...
    for(var i = 0; i<length; i=(i+1)|0){
        ...
        var subName = null;
        if(id.contains('/SUB:')){
        	var elems = id.split('/SUB:');
        	subName = elems[1];
        	var subData = data.params['s:'+subName] = data.params['s:'+subName]||{};
        	this.tryUpdateSpawnType(key,subData,data,subName);
        	subData[key] = i===0 ? args : args.concat();
        }else{
        	this.tryUpdateSpawnType(key,data.params,data,null);
		...
		//delete animation
		if(data.animations){
			for(var animIdx=data.animations.length-1; animIdx>=0; animIdx=(animIdx-1)|0){
				var animation = data.animations[animIdx];
				if(subName !== (animation.s||null))continue;
				if(animation.key!==key)continue;
				data.animations.splice(animIdx,1);
			}
		}
	}
};

Game_Particle.particleIdForUpdateSubEmitterParams = function(particleId,subConfigName){
	return particleId+'/SUB:'+subConfigName;
};


Game_Particle.prototype.tryUpdateSpawnType = function(type,params,data,subName=null){
	...
	Game_Particle.SPAWN_PARAMS.forEach(function(spawnKey){
		...
		delete params[spawnKey];
		if(animations){
			var length = animations.length;
		    for(var i = length-1; i>=0; i=(i-1)|0){
		        if(animations[i].key===spawnKey && animations[i].s===subName){
...
Game_Particle.prototype.particleAnimate = function(args){
    ...
    for(var i = 0; i<length; i=(i+1)|0){
		...
		if(!data.animations){
			...
			//delete same key animations
			for(var animIdx=data.animations.length-1; animIdx>=0; animIdx=(animIdx-1)|0){
				var anim = data.animations[animIdx];
				if(anim.s===subName && anim.key===key){
					data.animations.splice(animIdx,1);
				}
			}
		}
...
/* rotateHue
===================================*/
Game_Particle.prototype.rotateHue = function(id,hue=0,colorAdj=null,duration=0){
	var parData = this.dataWithId(id,true);
	if(!parData)return;

	var data = Game_Particle.configData(parData);
	var color = data.color;
	var targets = [];
	if(color.list){
		targets.push(...color.list);
	}else{
		targets.push({
			time:0,value:color.start
		},{
			time:1,value:color.end
		});
	}

	var args = [id,'color'];
	for(const target of targets){
		var {time,value} = target;
		var {r,g,b} = Game_Particle.rgbWithColor(value);
	    if(colorAdj){
	    	var rgb2 = Game_Particle.rgbWithColor(colorAdj);
	    	r *= rgb2.r/255;
	    	g *= rgb2.g/255;
	    	b *= rgb2.b/255;
	    }

		var {h,s,v} = Game_Particle.hsvWithRgb(r,g,b);
		h += hue;
		value = Game_Particle.colorWithHsv(h,s,v);
		args.push(value+'@'+time)
	}
	if(duration){
		args.splice(1,0,duration);
		this.particleAnimate(args);
	}else{
		this.particleUpdate(args);
	}
};
Game_Particle.rgbWithColor = function(value){
...
Game_Particle.hsvWithRgb = function(r,g,b){
    var h,s,v;
...
Game_Particle.colorWithHsv = function(h,s,v){
	var max = v;
...
Game_Particle.prototype.targetIds = function(idStr){
	...
	}else if(id_lc.indexOf('weather:')===0){
		var weatherId = idStr.substr(8);
		ret = this._keys.filter(key=>key.includes('_TRPW:'+weatherId+':'));
	}else{
		ret.push(idStr);
	}
	return ret;
...
ParticleEmitter.prototype._updateAnimation = function(animation){
	...
	var key = animation.key;
	if(animation.s){
		var subEmitter = this.subEmitter(animation.s);
		if(!subEmitter)return;
		this._updateParam(key,animation.args,rate,subEmitter);
	}else{
		this._updateParam(key,animation.args,rate);
	}
};
ParticleEmitter.prototype.subEmitter = function(){};
...
ParticleEmitter.prototype.updateParams = function(data,emitter=this._emitter){
    ...
    for(var i = 0; i<length; i=(i+1)|0){
        var key = keys[i];
        if(key.indexOf('s:')===0){
			this._updateSubEmitterParams(key,params[key],1);
		}else{
...
ParticleEmitter.prototype._updateSubEmitterParams = function(key,params,rate){};
