...
(function() {
    ...
    Sprite.prototype.addApngChild = function(name) {
        ...
        if (this._apngSprite) {
            if (this._apngSprite) {
                ...
                if (pixiApng) {
                    //pixiApng.stop();           // ← 自動再生を完全に止める
                    //pixiApng.autoPlay = false; // ← 自動再生フラグもオフ
                    //pixiApng.jumpToFrame(0);   // ← 最初のフレームへ
                }
            }
    ...
    Sprite.prototype.updateApngFrame = function () {
        if (this._apngStop) return;

        // 初回呼び出し時にタイマーを初期化
        if (this._apngTimer === undefined) {
            this._apngTimer = 0;
            this._apngLastUpdate = performance.now();
        }

        const now = performance.now();
        const elapsed = now - this._apngLastUpdate;
        const frameInterval = 1000 / 60 * param.FrameCount; // param.FrameCount フレームごと

        if (elapsed >= frameInterval) {
            this._apngLastUpdate = now;
            const frameLength = this._apngSprite.pixiApng.getFramesLength();
            this._apngTimer = (this._apngTimer + 1) % frameLength;
            this._apngSprite.pixiApng.jumpToFrame(this._apngTimer);
        }
    };
