 ...
 * @help
 * 【更新履歴】
 * 1.34 2024/12/13 updateコマンド対応
 *                 update 親ID/SUB:サブエミッター設定名 パラメータ...
...
ParticleEmitter.prototype.subEmitter = function(name){
	if(!isNaN(name)){
		name = '_sub:'+name+':'
			+ this._data.name.replace('/h','').replace(/_auto:[0-9]+:/,'')
			+'/h';
	}
...
ParticleEmitter.prototype._updateSubEmitterParams = function(key,params,rate){
	var subName = key.replace('s:','');
	var subEmitter = this.subEmitter(subName);
    ...
    for(var i = 0; i<length; i=(i+1)|0){
        var paramKey = keys[i];
        this._updateParam(paramKey,params[paramKey],rate,subEmitter);
    }
};
