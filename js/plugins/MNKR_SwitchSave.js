...
(() => {
  ...
  // キャンセル処理を「何もしない」関数に置き換えるための定義
  Scene_Save.prototype.onSaveFailure = function() {
    // ここでエラー音を鳴らしたい場合は AudioManager.playSe(...) を入れる
    // あえて「何もしない」ことで、キャンセル入力を無効化します
  };
  ...
  Scene_File.prototype.createListWindow = function () {
    ...
    // スイッチがON（あるいは未指定）の時だけキャンセルハンドラを上書きする
    const isSceneSave = this.constructor.name === 'Scene_Save';
    const isCancelNg = cancelSwitch === 0 ? true : $gameSwitches.value(cancelSwitch);

    if (isSceneSave && isCancelNg) {
