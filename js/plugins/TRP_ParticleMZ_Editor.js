 ...
 * @help
 * 【更新履歴】 
 * 1.34 2025/04/02 リスト表示の頭文字サーチの挙動変更
 * 1.32 2022/03/29 グループエディタでエスケープ文字(\)対応(\V[1]など変数使用可)
 * 1.31 2022/01/22 Ctrl+Shift+Sで別名で一度だけ保存する機能の追加
...
PickerBase.prototype.setTopRowNext = function(page=false){
	var delta = (page ? this._dispRows : 1);
	var index = (this._topRow-delta)*this.maxColumns()
	this.setTopIndex(index);
};
PickerBase.prototype.setTopRowPrevious = function(page=false){
	var delta = (page ? this._dispRows : 1);
	var index = (this._topRow+delta)*this.maxColumns();
	this.setTopIndex(index);
};
...
PickerBase.prototype.processInput = function(){
	if(Input.isTriggered('ok')||Input.isTriggered('cancel')){
		...
		SoundManager.playCursor();
		this.setTopRowNext(Input.isPressed('shift'));
	}else if(Input.isRepeated('down')){
		SoundManager.playCursor();
		this.setTopRowPrevious(Input.isPressed('shift'));
...
/* start picking
===================================*/
PresetPicker.prototype.startPicking = function(owner,targetType,currentData,lastName=null){
	this._categoryIndex = 0;
	...
	if(lastName){
		var idx = this._names.indexOf(lastName);
		if(idx>=0){
			this.didSuccessSearch(idx);
		}
	}
};
...
PresetPicker.prototype.processInput = function(){
	if(Input.isRepeated('cancel')){
		...
		this.selectNext(Input.isPressed('shift'));
	}else if(Input.isRepeated('up')){
		this.selectPrevious(Input.isPressed('shift'));
	}else{
		PickerBase.prototype.processInput.call(this);
	}
};
PresetPicker.prototype.selectNext = function(page){
	var idx0 = this._selectingIndexes.length ? this._selectingIndexes[0] : -1;
	var delta = (page ? this._dispRows+1 : 1);
	if(idx0===-1 && page){
		delta += 1;
	}
	var index = (idx0 + delta).clamp(0,this.maxItems()-1);
	if(index===idx0){
		SoundManager.playBuzzer();
		return;
	}
	...
	if(index > this._topRow+this._dispRows){
		this.setTopIndex(this._topRow+delta);
	}
};
PresetPicker.prototype.selectPrevious = function(page){
	var idx0 = this._selectingIndexes.length ? this._selectingIndexes[0] : -1;
	var delta = -(page ? this._dispRows+1 : 1);
	var index = (idx0 + delta).clamp(0,this.maxItems()-1);
	if(index===idx0){
